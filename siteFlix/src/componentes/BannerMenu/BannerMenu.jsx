...
function BannerMenu() {
  ...
  useEffect(() => {
    fetch(`${urlBase}popular?api_key=${apiKey}&language=pt-BR`)
      ...
      .then((data) => {
        const tresFilmes = data.results.slice(0, 3);
    ...
    >
      {filmes.map((filme, index) => (
        <div key={index} className="relative h-[100vh]">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-preto/100 via-preto/30 to-black/0 pointer-events-none z-10" />
